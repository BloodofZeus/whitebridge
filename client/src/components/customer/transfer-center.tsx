...
function HoldOverlay({ transfer, onResolved }: HoldOverlayProps) {
  ...
  const [holdPoint] = useState(72); // deterministic: stop at 72%
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-auto overflow-hidden">
        ...
        <div className="px-6 py-4">
          ...
          <div className="flex items-center justify-between mb-3">
            {[
              { label: 'Initiating', from: 0,  to: 25  },
              { label: 'Processing', from: 25, to: 55  },
              { label: 'Verifying',  from: 55, to: 72  },
              { label: 'Review',     from: 72, to: 100 },
            ].map(({ label, from, to }) => {
              const done = progress >= to;
              const active = progress >= from && progress < to;
              return (
                <div key={label} className="flex flex-col items-center gap-1">
                  <div className={`w-2 h-2 rounded-full transition-all ${done ? 'bg-emerald-500' : active ? progressBg + ' animate-pulse' : 'bg-slate-200'}`} />
                  <span className={`text-[9px] font-medium transition-colors ${done ? 'text-emerald-600' : active ? 'text-[#0A2D5E]' : 'text-slate-400'}`}>{label}</span>
                </div>
              );
            })}
          </div>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-medium text-slate-500">
              {resolved === 'approved' ? 'Approved' : resolved === 'rejected' ? 'Declined' : paused ? 'Security Review' : progress < 25 ? 'Initiating transfer…' : progress < 55 ? 'Processing payment…' : 'Running security checks…'}
            </span>
          ...
          {paused && !resolved && (
            <div className="mt-3 flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <p className="text-xs text-amber-700 font-medium">
                Security hold at 72% — A NorthBridge security agent is reviewing your transfer. Please respond below or wait.
              </p>
            </div>
          )}
        </div>
              ...
              <div className="h-40 overflow-y-auto p-3 space-y-2 bg-white">
                ...
                {chatMessages.map((msg) => (
                  ...
                  >
                    ...
                    >
                      {msg.isFromAdmin && (
                        <p className="text-[10px] font-bold text-[#C5003E] mb-0.5">NorthBridge Security</p>
                      )}
                      {msg.message}
                    </div>
                  </div>
